...
4. 다음처럼 입력합니다.
   - **Repository name:** `NEWXPAY` (원하면 다른 이름도 가능, 나중에 설정 한 군데만 바꾸면 됨)
...
3. GitHub 저장소와 연결합니다.  
   **`YOUR_USERNAME`** 자리에는 본인 GitHub 사용자 이름을 넣고,  
   저장소 이름을 **NEWXPAY**이 아닌 다른 이름으로 만들었다면 **NEWXPAY** 부분도 그 이름으로 바꿉니다.

```bash
git remote add origin https://github.com/YOUR_USERNAME/NEWXPAY.git
```
...
1. GitHub에서 방금 만든 저장소(**NEWXPAY**) 페이지로 갑니다.
...
2. **“Your site is live at…”** 아래에 나오는 주소가 배포된 주소입니다.
   - 형식: **`https://YOUR_USERNAME.github.io/NEWXPAY/`**
   - **끝에 `/` 와 `NEWXPAY/` 이 꼭 있어야** 합니다.
3. 이 주소를 브라우저에 넣어서 **실시간 매칭 시뮬레이터**가 뜨는지 확인합니다.
4. 이 주소를 다른 사람에게 보내면, 로그인 없이 같은 화면을 볼 수 있습니다.

---

## 저장소 이름을 NEWXPAY이 아닌 다른 이름으로 만든 경우

저장소 이름을 **예: my-matching-app** 처럼 다르게 만들었다면, 한 군데만 수정하면 됩니다.

1. **c:\NEWXPAY\vite.config.ts** 파일을 엽니다.
2. `base: '/NEWXPAY/',` 를 `base: '/my-matching-app/',` 처럼 **저장소 이름에 맞게** 바꿉니다.
...
- **Pages 주소에 마지막 `/` 를 빼고 들어가면** 404가 날 수 있습니다.  
  반드시 **`https://사용자명.github.io/NEWXPAY/`** 처럼 끝에 슬래시까지 포함해서 접속하세요.
...
| 단계 | 할 일 |
|------|--------|
| 1 | GitHub에서 **NEWXPAY** (또는 원하는 이름) **Public** 저장소 생성, README 추가 안 함 |
| 2 | `c:\NEWXPAY` 에서 `git init` → `add` → `commit` → `remote add origin` → `push` |
| 3 | 저장소 **Settings → Pages** 에서 Source를 **GitHub Actions** 로 설정 |
| 4 | **Actions** 탭에서 배포 완료(초록 체크) 될 때까지 대기 |
| 5 | **Settings → Pages** 에 나온 주소(`https://사용자명.github.io/NEWXPAY/`)로 접속해 확인 |
